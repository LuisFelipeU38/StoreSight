...
def test_video_upload(client):
    ...
    response = client.post(
        "/upload", data={"file": (io.BytesIO(b"video content"), "test.mp4")}
    )
    assert (
...
def test_video_upload_invalid_file_format(client):
    ...
    response = client.post(
        "/upload", data={"file": (io.BytesIO(b"invalid content"), "test.txt")}
    )
    assert (
        b"Invalid file format. Please upload a video file." in response.data
